...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5564993192287:+55 64 99319-2287\n' 
            + 'END:VCARD' 
prefix = '/'
blocked = []          
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiii @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}* leia as regras e comporte se pra n levar ban ^^`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["5564993192287@s.whatsapp.net"] 
...
case 'lofi':
                 ...
                 case 'logoph':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('escolha um txt')
					reply(mess.wait)
                ...
                case 'reportarbug':
                    ...
                    client.sendMessage('5564993192287@s.whatsapp.net', options, text, {quoted: mek})
                    reply('BUG REPORTADO AO CRIADOR!')
                    break
					case 'marcar':
					members_id = []
			...
			case 'mapa': 
				...
				case 'txtdafoto':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Escolha a foto para pegar o txt dela ${prefix} >-<')
					}
					break
				case 'stiker': 
				case 'sticker':
				...
				case 'figurinha':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`use o comando como legenda na foto ou coloque o comando marcando a imagem `)
					}
					break
				case 'gtts':	
				case 'tts':
				...
				case 'adicionar':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('coloque o número de quem deseja adicionar')
       ...
       client.sendMessage(from, 'Este é o número do meu criador ^-^ ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(` @${mentioned[0].split('@')[0]} rebaixado a membro comum`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
				case 'promover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `promovido a adm, parabéns :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`promovido a adm @${mentioned[0].split('@')[0]} parabéns`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
			...
			case 'remover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`n respeitou as regras toma ban @${mentioned[0].split('@')[0]} `, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'toimg':
				case 'converter':
					if (!isQuotedSticker) return reply('marca a fig q quer converte em imagem')
					reply(mess.wait)
